...
| `--cycle-kusa` | — | **循环模式**下轮流使用的草种，逗号分隔，如 `不灵草,灵灵草`；指定后每轮按顺序切换，忽略 `--kusa-type` |
...
### 不灵草 / 灵灵草轮流（`--cycle-kusa`）

在 `--loop` 下使用 `--cycle-kusa 不灵草,灵灵草` 时，脚本会按顺序轮流使用草种：

- 第 1 轮：不灵草（本轮不产草，为下一轮叠加翻倍）
- 第 2 轮：灵灵草（或你写的第二个草种，享受上一轮不灵草的翻倍）
- 第 3 轮：不灵草
- 第 4 轮：灵灵草
- …

每轮开始前会打印当前草种（如 `[本轮草种] 不灵草`）。也可指定更多草种，例如 `--cycle-kusa 不灵草,半灵草,巨草`，会按该顺序循环。
